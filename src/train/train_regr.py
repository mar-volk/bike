...
class TrainRegr:
    ...
    def __init__(self, model, exp_dir, x_train, y_train, x_val, y_val):
        ...
        self.y_train = torch.tensor(y_train, device=self.model.device, dtype=self.model.dtype) #.squeeze()
        self.x_val = torch.tensor(x_val, device=self.model.device, dtype=self.model.dtype)
        self.y_val = torch.tensor(y_val, device=self.model.device, dtype=self.model.dtype) #.squeeze()

        # define the name of the directory to be created
        if not os.path.isdir(self.exp_dir):
            os.makedirs(self.exp_dir)

    def run_train(self, n_epochs, lr=0.001, batch_size=16, weight_decay=0):
        # set model to training-mode
        self.model.train()

        # Loss and optimizer
        criterion = torch.nn.MSELoss() #torch.nn.AbsCriterion()
